...
<file>              path relative to the repo root (e.g. src/lemmas/ring_lemmas.rs)
[function_pattern]  optionally restrict verification to matching functions

examples:
  ./scripts/check-single-file.sh src/traits/ring.rs
  ./scripts/check-single-file.sh src/lemmas/field_lemmas.rs
  ./scripts/check-single-file.sh src/lemmas/ordered_ring_lemmas.rs lemma_square_nonneg
  ./scripts/check-single-file.sh src/traits/equivalence.rs
EOF
  exit 0
fi
...
#   src/lemmas/ring_lemmas.rs  ->  lemmas::ring_lemmas
...
run_cargo_verus() {
  if command -v rustup >/dev/null 2>&1; then
    ...
    cd "$ROOT_DIR"
    cargo verus verify --manifest-path Cargo.toml -p verus-algebra \
