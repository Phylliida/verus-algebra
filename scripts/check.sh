...
check_ci_workflow_checkout_wiring() {
  ...
  if [[ -z "$checkout_algebra_line" || -z "$checkout_verus_line" ]]; then
    echo "error: required checkout steps missing in $workflow_file"
    echo "expected steps: 'Checkout verus-algebra' and 'Checkout Verus'"
    exit 1
  fi
  ...
  if [[ -z "$checkout_algebra_step" || -z "$checkout_verus_step" ]]; then
...
check_ci_workflow_end_to_end_structure() {
  ...
  if ! printf '%s\n' "$strict_step" | grep -qE 'working-directory:[[:space:]]*verus-algebra'; then
    echo "error: workflow 'Run strict checks' step must run in verus-algebra"
    exit 1
  fi
...
run_cargo_verus_verify() {
  ...
  if command -v rustup >/dev/null 2>&1; then
    ...
    if [[ -n "$feature_flags" ]]; then
      ...
      "${CARGO_CMD[@]}" verus verify --manifest-path Cargo.toml -p verus-algebra $feature_flags -- --triggers-mode silent
    else
      "${CARGO_CMD[@]}" verus verify --manifest-path Cargo.toml -p verus-algebra -- --triggers-mode silent
    fi
    ...
    if nix-shell -p rustup --run "rustup --version >/dev/null 2>&1" >/dev/null 2>&1; then
      ...
      nix-shell -p rustup --run "
        ...
        export VERUS_Z3_PATH='$VERUS_SOURCE/z3'
        cd '$ROOT_DIR'
        cargo $OFFLINE_CARGO_FLAG verus verify --manifest-path Cargo.toml -p verus-algebra $feature_flags -- --triggers-mode silent
      "
    else
      skip_or_fail_verus_unavailable \
